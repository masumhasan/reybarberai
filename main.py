...
async def debug_search(
    ...
    """
    Debug endpoint — shows similarity scores for ALL documents,
    regardless of threshold, so you can see why matches are missed.
    """
    image_bytes = await file.read()
    query_vector = embedder.embed_from_bytes(image_bytes)
    query_vector = query_vector / np.linalg.norm(query_vector)

    rows = []
    for doc in db.col.find({}, {"shop_id":1,"barber_id":1,"image_name":1,"embedding":1}):
        raw = doc.get("embedding")
        if not raw:
            rows.append({"_id": str(doc["_id"]), "error": "no embedding field"})
            continue

        stored = np.array(raw, dtype=np.float32)
        norm = np.linalg.norm(stored)
        if norm == 0:
            rows.append({"_id": str(doc["_id"]), "error": "zero-norm vector"})
            continue
        ...
        rows.append({
            ...
            "embedding_length": len(raw),
        })

    rows.sort(key=lambda x: x.get("similarity", -1), reverse=True)

    return {
        "query_image":    file.filename,
        "total_docs":     len(rows),
        "query_vec_norm": float(np.linalg.norm(query_vector)),
        "results":        rows,
    }
...
async def debug_db():
    """Shows DB stats — total docs, sample doc shape, embedding presence."""
    total = db.col.count_documents({})
    sample = db.col.find_one({}, {"embedding": 1, "shop_id": 1,
                                   "barber_id": 1, "image_name": 1})
    has_embedding = False
    embedding_length = 0
    ...
    return {
        ...
        "sample_shop_id":   sample.get("shop_id") if sample else None,
        "sample_image_name":sample.get("image_name") if sample else None,
    }
